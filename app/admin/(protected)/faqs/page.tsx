import Link from "next/link";
import { adminFetch } from "@/lib/admin-auth";
import type { SiteFaq } from "@/lib/api";
import { deleteFaq } from "@/lib/actions/faqs";

export default async function AdminFaqsPage() {
  const res = await adminFetch("/api/sitefaqs");
  const faqs: SiteFaq[] = res.ok ? await res.json() : [];

  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="font-display font-bold text-2xl text-text">FAQs</h1>
        <Link
          href="/admin/faqs/new"
          className="px-4 py-2 bg-accent hover:bg-[var(--color-accent-hover)] text-white text-sm font-semibold rounded-lg transition-colors"
        >
          New FAQ
        </Link>
      </div>

      <div className="bg-surface border border-border rounded-2xl overflow-hidden">
        {faqs.length === 0 ? (
          <p className="p-6 text-muted text-sm">No FAQs yet.</p>
        ) : (
          <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border text-left text-muted uppercase text-xs tracking-wider">
                <th className="px-5 py-3">Question</th>
                <th className="px-5 py-3">Answer</th>
                <th className="px-5 py-3"></th>
              </tr>
            </thead>
            <tbody>
              {faqs.map((f) => (
                <tr key={f.id} className="border-b border-border last:border-0">
                  <td className="px-5 py-3 text-text font-medium max-w-xs truncate">{f.question}</td>
                  <td className="px-5 py-3 text-muted max-w-sm truncate">{f.answer}</td>
                  <td className="px-5 py-3 text-right whitespace-nowrap">
                    <Link
                      href={`/admin/faqs/${f.id}/edit`}
                      className="text-accent hover:underline mr-4"
                    >
                      Edit
                    </Link>
                    <form action={deleteFaq.bind(null, f.id)} className="inline">
                      <button type="submit" className="text-red-500 hover:underline">
                        Delete
                      </button>
                    </form>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
          </div>
        )}
      </div>
    </div>
  );
}
